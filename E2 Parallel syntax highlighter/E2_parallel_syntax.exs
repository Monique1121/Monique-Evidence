# E2 Syntax highlighter - Parallel version
#
...
defmodule TecFiles do
  ...
  def syntax(filename) do
    ...
    <html>
      <head>
        <title>#{Path.rootname(Path.basename(filename))}_parallel</title>
    ...
    # Path.basename removes the directory path
    # Path.rootname removes the extension
    File.write("Test_Files_Result_Parallel/#{Path.rootname(Path.basename(filename))}_parallel.html", [header, document, "\n", footer])
  end

  # Function that processes all files in the given directory in parallel
  # Each file is launched as an independent process using Task.async and waited on with Task.await,
  # distributing work across available CPU cores
  def read_files_parallel(directory) do

    {:ok, files} = File.ls(directory)
    files
    |> Enum.map(&Task.async(fn -> syntax("#{directory}/#{&1}") end))
